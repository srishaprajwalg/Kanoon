...
export const LegalRiskBriefModal: React.FC<LegalRiskBriefModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/85 backdrop-blur-md flex items-center justify-center p-3 sm:p-6 overflow-y-auto no-print">
      <div className="glass-panel w-full max-w-4xl rounded-3xl border border-slate-700 p-5 sm:p-8 max-h-[92vh] flex flex-col justify-between shadow-2xl relative animate-fadeIn">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-800 pb-4 gap-3">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <div className="flex items-center space-x-2">
                <h2 className="font-extrabold text-slate-100 text-lg">Advocate Legal Risk Brief</h2>
          ...
          <div className="flex items-center space-x-2 self-end sm:self-auto">
            <button
              ...
              className="flex items-center space-x-1.5 px-3 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-xs font-medium text-slate-200 border border-slate-700 transition-colors"
            ...
            <button
              ...
              className="flex items-center space-x-1.5 px-3 py-1.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-slate-950 text-xs font-bold shadow transition-colors"
            ...
            <button
              onClick={handlePrint}
              className="flex items-center space-x-1.5 px-3 py-1.5 rounded-xl bg-slate-900 border border-slate-700 text-slate-300 hover:bg-slate-800 text-xs font-medium"
              title="Print brief"
            >
        ...
        <div className="my-3 p-3.5 rounded-2xl bg-slate-900/90 border border-amber-500/20 flex flex-col md:flex-row md:items-center justify-between gap-3 text-xs">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <div className="flex items-center space-x-2">
                <span className="font-bold text-slate-200">Handoff Status:</span>
                <span className="text-[11px] px-2 py-0.5 rounded bg-emerald-500/10 text-emerald-300 border border-emerald-500/20 font-semibold uppercase">
        ...
        {/* Tab Navigation */}
        <div className="flex border-b border-slate-800 text-xs font-semibold gap-6 mb-3">
          <button
            onClick={() => setActiveTab('preview')}
            className={`pb-2 border-b-2 transition-colors ${
              activeTab === 'preview' ? 'border-amber-500 text-amber-400' : 'border-transparent text-slate-400 hover:text-slate-200'
            }`}
          >
          ...
          <button
            onClick={() => setActiveTab('advocates')}
            className={`pb-2 border-b-2 transition-colors ${
              activeTab === 'advocates' ? 'border-amber-500 text-amber-400' : 'border-transparent text-slate-400 hover:text-slate-200'
            }`}
          >
          ...
          <button
            onClick={() => setActiveTab('notes')}
            className={`pb-2 border-b-2 transition-colors ${
              activeTab === 'notes' ? 'border-amber-500 text-amber-400' : 'border-transparent text-slate-400 hover:text-slate-200'
            }`}
          >
          ...
          {activeTab === 'preview' && (
            <div className="space-y-6">
              ...
              <div className="glass-panel p-5 rounded-2xl border border-slate-800 space-y-3">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs bg-slate-950 p-3 rounded-xl border border-slate-850">
                  <div>
                    <span className="text-slate-500 block">Document Title</span>
                    <strong className="text-slate-200 font-semibold">{brief.documentTitle}</strong>
                  </div>
                  <div>
                    <span className="text-slate-500 block">Jurisdiction</span>
                    <strong className="text-slate-200 font-semibold">{brief.jurisdiction}</strong>
                  </div>
                  <div>
              ...
              <div className="space-y-3">
                ...
                {brief.criticalIssues.length === 0 ? (
                  <div className="p-4 rounded-xl bg-slate-900 border border-slate-800 text-xs text-slate-400 flex items-center space-x-2">
                  ...
                  <div className="space-y-3">
                    {brief.criticalIssues.map((item, idx) => (
                      <div key={idx} className="glass-panel p-4 rounded-xl border border-rose-500/20 space-y-2 text-xs">
                        <div className="flex items-center justify-between">
                        ...
                        <p className="text-slate-300">
                          <strong className="text-slate-400">Plain Explanation:</strong> {item.plainEnglishExplanation}
                        </p>

                        <div className="bg-slate-950 p-2.5 rounded-lg border border-slate-800 space-y-1">
                          <span className="text-amber-400 font-bold text-[11px] block">Recommended Action:</span>
                          ...
                          {item.saferAlternative && (
                            <div className="pt-1.5 border-t border-slate-850 mt-1">
                              ...
                              <p className="text-emerald-300 font-mono text-[11px] italic">"{item.saferAlternative}"</p>
                            </div>
                          )}
                        </div>
                      </div>
                ...
                {brief.missingProvisions.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {brief.missingProvisions.map((m, idx) => (
                      <div key={idx} className="glass-panel p-3.5 rounded-xl border border-amber-500/20 space-y-1.5 text-xs">
                        <div className="flex items-center justify-between">
                          <span className="font-bold text-slate-100">{m.clauseType}</span>
                          <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-amber-500/10 text-amber-400 uppercase">
                            {m.importance}
                          </span>
                        </div>
                        <p className="text-slate-300 text-[11px]">{m.whyItMatters}</p>
                        <p className="text-amber-300/90 text-[11px] font-medium pt-1 border-t border-slate-800">{m.recommendedAction}</p>
                      </div>
                    ))}
                  </div>
                )}
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    {brief.inconsistencies.map((inc, idx) => (
                      <div key={idx} className="glass-panel p-3.5 rounded-xl border border-orange-500/20 text-xs space-y-1">
                        <span className="font-bold text-slate-100">{inc.issueTitle}</span>
                        <p className="text-slate-300">{inc.explanation}</p>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-slate-400 uppercase font-semibold">Tier 1 Gazette Sources</span>
                </div>
                      ...
                      <div key={idx} className="glass-panel p-4 rounded-xl border border-slate-800 space-y-2 text-xs">
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1">
                          ...
                          {cit.similarityScore !== undefined && (
                            <span className="px-2 py-0.5 rounded bg-amber-500/10 text-amber-400 border border-amber-500/20 text-[10px] font-mono font-bold self-start sm:self-auto">
                              Similarity: {(cit.similarityScore * 100).toFixed(0)}%
                            </span>
                          )}
                        </div>

                        <p className="text-slate-300 font-serif bg-slate-950 p-3 rounded-lg border border-slate-850 text-[11px] italic">
                          "{cit.statuteText}"
                        </p>
          ...
          {activeTab === 'advocates' && (
            <div className="space-y-4">
              <div className="p-4 rounded-2xl bg-slate-900 border border-slate-800 space-y-1">
                <h3 className="text-sm font-bold text-slate-100">Select Advocate for Professional Handover</h3>
                <p className="text-xs text-slate-400">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {ADVOCATES_DIRECTORY.map((advocate: AdvocateProfile) => {
                  ...
                  return (
                    <div
                      key={advocate.id}
                      className={`glass-panel p-4 rounded-2xl border transition-all flex flex-col justify-between space-y-3 ${
                        isSelected
                    ...
                    >
                      <div className="flex items-start space-x-3">
                        ...
                        <div className="space-y-0.5 flex-1">
                          <h4 className="font-bold text-slate-100 text-sm">{advocate.name}</h4>
                          <p className="text-[11px] text-amber-400">{advocate.title}</p>
                          <p className="text-[11px] text-slate-400">
                      ...
                      <div className="flex items-center justify-between pt-2 border-t border-slate-800 text-xs">
                        <span className="font-extrabold text-amber-400">₹{advocate.consultationFee} / 30m</span>
                        <button
                          onClick={() => {
          ...
          {activeTab === 'notes' && (
            <div className="space-y-4">
              ...
              <textarea
                ...
                className="w-full bg-slate-950 border border-slate-700 rounded-2xl p-4 text-xs text-slate-100 focus:outline-none focus:border-amber-500 leading-relaxed font-sans"
              />
        ...
        <div className="pt-3 border-t border-slate-800 text-[11px] text-slate-500 flex flex-col sm:flex-row items-center justify-between gap-2">
          ...
          <span className="font-mono text-slate-400">Kanoon AI v2.0 Legal Brief</span>
        </div>
      </div>
    </div>
  );
