    ...
    <div className="space-y-8 pb-12">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 relative z-10">
          <div className="space-y-2 max-w-2xl">
            ...
            <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-100">
              Connect with Experienced <span className="bg-gradient-to-r from-amber-400 to-amber-500 bg-clip-text text-transparent">Bar Council Advocates</span>
            </h1>
          ...
          {/* City Filter */}
          <div className="bg-slate-900/90 p-3 rounded-xl border border-slate-800 self-stretch md:self-auto space-y-1">
            <label className="text-[11px] font-bold text-slate-400 block uppercase">Filter Advocates by City:</label>
            <select
              value={selectedCity}
              onChange={(e) => setSelectedCity(e.target.value)}
              className="bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-xs text-slate-100 focus:outline-none focus:border-amber-500 w-full"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredAdvocates.map((advocate: AdvocateProfile) => (
          <div
            key={advocate.id}
            className="glass-card rounded-2xl p-5 border border-slate-800 flex flex-col justify-between space-y-4 hover:border-amber-500/40 transition-all"
          >
            <div className="space-y-4">
              <div className="flex items-start space-x-4">
                ...
                <div className="space-y-1 flex-1">
                  <div className="flex items-center justify-between">
                    <h3 className="font-bold text-slate-100 text-base">{advocate.name}</h3>
                    <span className="flex items-center space-x-1 text-xs text-amber-400 font-bold bg-amber-500/10 px-2 py-0.5 rounded">
                      <Star className="w-3.5 h-3.5 fill-amber-400 text-amber-400" />
                      <span>{advocate.rating}</span>
                    </span>
                  </div>
                  <p className="text-xs text-amber-400/90 font-medium">{advocate.title}</p>
              ...
              <div className="pt-3 border-t border-slate-800/80 space-y-2 text-xs">
                <div className="flex items-center justify-between text-slate-400">
                  ...
                  <strong className="text-slate-200">{advocate.experienceYears}+ Years</strong>
                </div>
                <div className="flex items-center justify-between text-slate-400">
                  <span>Bar Enrollment:</span>
                  <strong className="text-slate-300 font-mono text-[11px]">{advocate.barCouncilNumber}</strong>
                </div>
            ...
            <div className="pt-4 border-t border-slate-800 flex items-center justify-between">
              <div>
                ...
                <span className="text-sm font-extrabold text-amber-400">₹{advocate.consultationFee.toLocaleString('en-IN')}<span className="text-[10px] text-slate-400"> / 30m</span></span>
              </div>

              <button
                onClick={() => setSelectedAdvocate(advocate)}
                className="flex items-center space-x-1.5 px-4 py-2 rounded-xl bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-xs shadow transition-colors"
              >
      ...
      {selectedAdvocate && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="glass-panel max-w-md w-full rounded-2xl p-6 border border-slate-700 space-y-4 relative animate-fadeIn">
            <button
            ...
            <div className="space-y-1">
              <h3 className="text-lg font-bold text-slate-100">Schedule Consultation</h3>
              <p className="text-xs text-slate-400">Booking 30-minute legal advice session with <strong>{selectedAdvocate.name}</strong></p>
            </div>

            {bookingSuccess ? (
              <div className="p-6 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-center space-y-2">
                <CheckCircle2 className="w-10 h-10 text-emerald-400 mx-auto" />
                <h4 className="font-bold text-emerald-300 text-sm">Consultation Requested!</h4>
              ...
              <form onSubmit={handleBookingSubmit} className="space-y-3 text-xs">
                ...
                <div>
                  <label className="block text-slate-400 mb-1 font-medium">Brief Description of Legal Matter</label>
                  <textarea
                    rows={3}
                ...
                <div className="pt-2 flex justify-end space-x-2">
                  ...
                  <button
                    type="submit"
                    className="px-5 py-2 rounded-lg bg-amber-500 hover:bg-amber-400 text-slate-950 text-xs font-bold shadow"
                  >
