...
export function App() {
  ...
  return (
    <div className="min-h-screen bg-stone-50 text-slate-900 flex flex-col font-sans selection:bg-legal-100 selection:text-legal-900">
      {/* Top Navbar */}
      <Header
        activeTab={activeTab}
      ...
      <footer className="border-t border-slate-200 bg-white text-slate-600 text-xs py-8 no-print mt-auto">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="flex items-center space-x-3">
              <div className="w-8 h-8 rounded-lg bg-legal-50 text-legal-700 flex items-center justify-center font-bold border border-legal-200">
                <Scale className="w-4 h-4" />
              </div>
              <span className="font-serif font-bold text-slate-800 text-sm tracking-tight">Kanoon AI</span>
              <span className="text-slate-400">|</span>
              <span className="text-slate-600 font-medium">Legal Documentation Assistant</span>
            </div>

            <div className="flex items-center space-x-4 text-slate-500">
              <button
                onClick={() => setActiveTab('database')}
                className="hover:text-legal-700 flex items-center space-x-1.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-legal-600 rounded"
              >
          ...
          <div className="p-5 rounded-xl bg-stone-50 border border-stone-200 text-[13px] leading-relaxed text-slate-700 flex items-start space-x-4 shadow-sm">
            <ShieldAlert className="w-5 h-5 text-amber-600 flex-shrink-0 mt-0.5" aria-hidden="true" />
            <div className="space-y-1">
              <p className="font-semibold text-slate-900">Ethical AI &amp; Legal Disclaimer</p>
              <p>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between text-xs text-slate-500 pt-4 border-t border-slate-100">
            ...
            <span className="flex items-center space-x-1.5 mt-2 sm:mt-0 font-medium">
              ...
              <Heart className="w-3.5 h-3.5 text-red-500 fill-red-500" aria-label="Heart icon" />
            </span>
          </div>
        </div>
      </footer>
